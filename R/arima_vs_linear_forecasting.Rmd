...
*  `viewers ~ week` in orange
* 

Using just the year gives the gradual decline over all we would expect, whilst using just the quarter shows the seasonal variation. The  combination of the two gives the best forecast showing both factors. This combined forecast is very similar to the simple `viewers ~ week_commencing` model plotted in black, but it is still far higher than the forecast using pre-covid data plotted in blue.
...
model_yr_week <- lm(data = tv, formula = viewers ~ year + week)
future_dates<- x_dates %>% filter(week_commencing > max(tv$week_commencing))
forecast_yr_week <-
  cbind(future_dates) %>%
  data.frame(viewers =  predict(model_yr_week, future_dates)) #%>%
  #rbind(tv %>% select(year, quarter, year_quarter, week_commencing, viewers))
...
lm_model_qtr <- lm(data = tv, formula = viewers ~ year + quarter + covid)
future_dates<- x_dates %>% filter(week_commencing > max(tv$week_commencing)) %>% mutate(covid = 0)
forecast_qtr <-
  cbind(future_dates) %>%
  data.frame(viewers =  predict(lm_model_qtr, future_dates))

lm_model_wk <- lm(data = tv, formula = viewers ~ year + week + covid)
future_dates<- x_dates %>% filter(week_commencing > max(tv$week_commencing)) %>% mutate(covid = 0)
forecast_wk <-
  cbind(future_dates) %>%
  data.frame(viewers =  predict(lm_model_wk, future_dates))
...
```{r tv_log, echo=FALSE}
# tv<-tv %>% mutate(viewers = log(viewers) )

lm_model <- lm(data = tv, formula = log(viewers) ~ year + quarter + covid)
future_dates<- x_dates %>% filter(week_commencing > max(tv$week_commencing)) %>% mutate(covid = 0)
forecast <-
  cbind(future_dates) %>%
  data.frame(viewers =  exp(predict(lm_model, future_dates)))
